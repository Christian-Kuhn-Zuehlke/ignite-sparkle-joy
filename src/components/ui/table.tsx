...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn("h-11 border-b border-muted-subtle transition-colors data-[state=selected]:bg-background hover:bg-muted", className)}
      {...props}
    />
  ),
);
...
const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        "h-11 px-3 text-left align-middle text-sm font-medium uppercase tracking-wider text-secondary [&:has([role=checkbox])]:pr-0",
        className,
      )}
      {...props}
    />
...
const TableCaption = React.forwardRef<HTMLTableCaptionElement, React.HTMLAttributes<HTMLTableCaptionElement>>(
  ({ className, ...props }, ref) => (
    <caption ref={ref} className={cn("mt-4 text-sm text-subtitle", className)} {...props} />
  ),
);
